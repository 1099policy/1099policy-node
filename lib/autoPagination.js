...
function makeAutoPaginationMethods(self, requestArgs, spec, firstPagePromise) {
  ...
  if (spec.methodType === 'search') {
    getNextPagePromise = (pageResult) => {
      if (!pageResult.next_page) {
        throw Error(
          'Unexpected: Ten99Policy API response does not have a well-formed `next_page` field, but `has_more` was true.'
        );
      }
  ...
  function iterate(pageResult) {
    if (
      ...
      throw Error(
        'Unexpected: Ten99Policy API response does not have a well-formed `data` array.'
      );
    }
