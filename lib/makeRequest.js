...
function getRequestOpts(self, requestArgs, spec, overrideData) {
  ...
  const urlData = urlParams.reduce((urlData, param) => {
    ...
    if (typeof arg !== 'string') {
      throw new Error(
        `Ten99policy: Argument "${param}" must be a string, but got: ${arg} (on API request to \`${requestMethod} ${path}\`)`
      );
    }
  ...
  if (args.filter((x) => x != null).length) {
    throw new Error(
      `Ten99policy: Unknown arguments (${args}). Did you mean to pass an options object? See https://github.com/ten99policy/ten99policy-node/wiki/Passing-Options. (on API request to ${requestMethod} \`${path}\`)`
    );
  }
