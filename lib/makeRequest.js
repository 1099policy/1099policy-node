...
function getRequestOpts(self, requestArgs, spec, overrideData) {
  ...
  const data = JSON.stringify(encode(Object.assign({}, dataFromArgs, overrideData)));
  const options = utils.getOptionsFromArgs(args);

  // Validate that there are no more args.
  if (args.filter((x) => x != null).length) {
    throw new Error(
      `Ten99Policy: Unknown arguments (${args}). Did you mean to pass an options object?`
    );
  }
...
function makeRequest(self, requestArgs, spec, overrideData) {
  return new Promise((resolve, reject) => {
    ...
    // T99K api accepts application/json
