...
class Resource {
  ...
  logCurlCommand(method, url, headers, data = null) {
    ...
    /**
     * Appends HTTP headers to a cURL command string
     * @param {Object} headers - An object containing key-value pairs of HTTP headers
     * @param {string} curl - The existing cURL command string to append headers to
     * @returns {string} Updated cURL command string with added headers
     */
...
/**
 * Iterates through an object's properties, attempting to parse string values that appear to be JSON.
 * @param {Object} data - The object containing potentially JSON-formatted string values.
 * @returns {void} This method doesn't return a value, it modifies the input object in place.
 */
  ...
  unpackJsonFields(data) {
    if (Array.isArray(data)) {
      /**
       * Maps over an array of data items and unpacks JSON fields for each item.
       * @param {Array} data - An array of objects containing JSON fields to be unpacked.
       * @returns {Array} A new array with JSON fields unpacked for each item.
       */
  ...
  list(params, requestHeaders = {}) {
    ...
    return axios
      ...
      /**
       * Unpacks JSON fields from the response data
       * @param {Object} response - The response object containing data to unpack
       * @returns {Object} The unpacked data with JSON fields parsed
       */
      .then(response => this.unpackJsonFields(response.data)) // Unpack JSON fields
      .catch(error => this.handleApiError(error));
  /**
   * Unpacks JSON fields from the response data
   * @param {Object} response - The response object containing data to be unpacked
   * @returns {Object} The unpacked data with JSON fields parsed
   */
  }
