...
// to the operation of ten99policy-node, we handle this unavailability gracefully.
let exec = null;
try {
...
const DEPRECATED_OPTIONS = {
  ...
  ten99policy_account: 'ten99policyAccount',
  ten99policy_version: 'apiVersion',
  ten99policyVersion: 'apiVersion',
};
...
const utils = (module.exports = {
    ...
    if (
      ...
      emitWarning(
        `Options found in arguments (${optionKeysInArgs.join(
          ', '
        )}). Did you mean to pass an options object? See https://github.com/ten99policy/ten99policy-node/wiki/Passing-Options.`
      );
    }
  ...
  getOptionsFromArgs: (args) => {
    ...
    if (args.length > 0) {
      ...
      if (typeof arg === 'string') {
        ...
        if (params.ten99policyAccount) {
          opts.headers['Ten99policy-Account'] = params.ten99policyAccount;
        }
        if (params.ten99policyEnvironment) {
          opts.headers['Ten99policy-Environment'] = params.ten99policyEnvironment;
        }
        if (params.apiVersion) {
          opts.headers['Ten99policy-Version'] = params.apiVersion;
        }
  ...
  protoExtend(sub) {
    ...
    // context:
    //
    // https://github.com/ten99policy/ten99policy-node/pull/334
...
function emitWarning(warning) {
  ...
  return process.emitWarning(warning, 'Ten99policy');
}
