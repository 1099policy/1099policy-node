...
Ten99Policy.PACKAGE_VERSION = require('../package.json').version;

const utils = require('./utils');
const {determineProcessUserAgentProperties, emitWarning} = utils;

Ten99Policy.USER_AGENT = {
  bindings_version: Ten99Policy.PACKAGE_VERSION,
  lang: 'node',
...
Ten99Policy.resources = resources;

const {HttpClient, HttpClientResponse} = require('./net/HttpClient');
Ten99Policy.HttpClient = HttpClient;
Ten99Policy.HttpClientResponse = HttpClientResponse;

const CryptoProvider = require('./crypto/CryptoProvider');
Ten99Policy.CryptoProvider = CryptoProvider;
...
function Ten99Policy(config = {}) {
  ...
  this.VERSION = Ten99Policy.PACKAGE_VERSION;
  ...
  if (
    ...
    (!props.host || /\.1099policy\.com$/.test(props.host))
  ) {
    throw new Error(
      'The `https` protocol must be used when sending requests to `*.1099policy.com`'
    );
  }
  ...
  this._api = {
    ...
    agent: agent,
    httpClient: props.httpClient || Ten99Policy.createNodeHttpClient(agent),
    dev: false,
    ten99policyAccount: props.ten99policyAccount || null,
    environment: props.environment || DEFAULT_API_ENVIRONMENT,
  };

  const typescript = props.typescript || false;
  if (typescript !== Ten99Policy.USER_AGENT.typescript) {
    // The mutation here is uncomfortable, but likely fastest;
    // serializing the user agent involves shelling out to the system,
    // and given some users may instantiate the library many times without switching between TS and non-TS,
    // we only want to incur the performance hit when that actually happens.
    Ten99Policy.USER_AGENT.typescript = typescript;
  }
  ...
  // Expose Ten99PolicyResource on the instance too
  this.Ten99PolicyResource = Ten99Policy.Ten99PolicyResource;
}

Ten99Policy.errors = require('./Error');
Ten99Policy.webhooks = require('./Webhooks');

Ten99Policy.createNodeHttpClient = (agent) => {
  const {NodeHttpClient} = require('./net/NodeHttpClient');
  return new NodeHttpClient(agent);
};

/**
 * Creates an HTTP client for issuing Ten99Policy API requests which uses the Web
 * Fetch API.
 *
 * A fetch function can optionally be passed in as a parameter. If none is
 * passed, will default to the default `fetch` function in the global scope.
 */
Ten99Policy.createFetchHttpClient = (fetchFn) => {
 ...
 */
Ten99Policy.createNodeCryptoProvider = () => {
 ...
 * scope.
 */
Ten99Policy.createSubtleCryptoProvider = (subtleCrypto) => {
...
Ten99Policy.prototype = {
   ...
   *  apiKey: '<api-key>',  // requiredkey: api-key,
   *   host: 'example.com',
   *   port: '8080',
   *   protocol: 'http',
   * });
   ...
   * @deprecated will be removed in a future major version. Use the config object instead:
   *
   * const ten99policy = new Ten99Policy(API_KEY);
   *
   * Or, for Ten99Policy Connect, use `ten99policyAccount` instead:
   *
   * const ten99policy = new Ten99Policy({
   *   key: api-key,
   *   ten99policyAccount: 'acct_...',
   * });
   *
   * Or, to use a different apiKey on a given request:
   *
   * ten99policy.customers.create(params, {apiKey: 'sk_test_...'});
   */
  setApiKey(key) {
    emitWarning(
  ...
  /**
   * @private
   * Please open or upvote an issue at github.com/ten99policy/ten99policy-node
   ...
   */
  getUname(cb) {
    if (!Ten99Policy._UNAME_CACHE) {
      Ten99Policy._UNAME_CACHE = new Promise((resolve) => {
        utils.safeExec('uname -a', (err, uname) => {
          resolve(uname);
        });
      });
    }
    Ten99Policy._UNAME_CACHE.then((uname) => cb(uname));
  },
  ...
  getClientUserAgent(cb) {
    return this.getClientUserAgentSeeded(Ten99Policy.USER_AGENT, cb);
  },
...
module.exports.Ten99Policy = Ten99Policy;

// Allow use with the TypeScript compiler without `esModuleInterop`.
// We may also want to add `Object.defineProperty(exports, "__esModule", {value: true});` in the future, so that Babel users will use the `default` version.
module.exports.default = Ten99Policy;
