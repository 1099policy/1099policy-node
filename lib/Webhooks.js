...
const {Ten99policyError, Ten99policySignatureVerificationError} = require('./Error');
...
const Webhook = {
   ...
   * @property {string} secret - Ten99policy webhook secret 'whsec_...'
...
function parseEventDetails(encodedPayload, encodedHeader, expectedScheme) {
  ...
  if (Array.isArray(encodedHeader)) {
    throw new Error(
      'Unexpected: An array was passed as a header, which should not be possible for the ten99policy-signature header.'
    );
  }
  ...
  if (!details || details.timestamp === -1) {
    throw new Ten99policySignatureVerificationError({
  ...
  if (!signatureFound) {
    throw new Ten99policySignatureVerificationError({
      message:
        ...
        ' Are you passing the raw request body you received from Ten99policy?' +
        ' https://github.com/ten99policy/ten99policy-node#webhook-signing',
      detail: {
        header,
        payload,
      },
